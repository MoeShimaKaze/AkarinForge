...
public class ServerWorldEventHandler implements IWorldEventListener
{
    ...
    {
        this.mcServer.getPlayerList().sendToAllNearExcept(player, x, y, z, volume > 1.0F ? (double)(16.0F * volume) : 16.0D, this.world.dimension, new SPacketSoundEffect(soundIn, category, x, y, z, volume, pitch)); // CraftBukkit - this.world.dimension
    }
    ...
    {
        this.mcServer.getPlayerList().sendToAllNearExcept(player, (double)blockPosIn.getX(), (double)blockPosIn.getY(), (double)blockPosIn.getZ(), 64.0D, this.world.dimension, new SPacketEffect(type, blockPosIn, data, false));// CraftBukkit - this.world.dimension
    }
    ...
    public void sendBlockBreakProgress(int breakerId, BlockPos pos, int progress)
    {
        ...
        if (entity instanceof EntityPlayer) sourcePlayer = (EntityPlayer) entity;
        // CraftBukkit end
        for (EntityPlayerMP entityplayermp : this.mcServer.getPlayerList().getPlayers())
        {
            if (entityplayermp != null && entityplayermp.world == this.world && entityplayermp.getEntityId() != breakerId)
            {
                ...
                if (sourcePlayer != null && sourcePlayer instanceof EntityPlayerMP && !entityplayermp.getBukkitEntity().canSee(((EntityPlayerMP) sourcePlayer).getBukkitEntity())) continue; // CraftBukkit
