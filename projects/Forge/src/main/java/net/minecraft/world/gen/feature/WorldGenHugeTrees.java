        ...
        if (leavesPos.getY() >= 1 && leavesPos.getY() + height + 1 <= 256)
        {
            for (int i = 0; i <= 1 + height; ++i)
            {
                ...
                for (int k = -j; k <= j && flag; ++k)
                {
                    ...
                    {
                        if (leavesPos.getY() + i < 0 || leavesPos.getY() + i >= 256 || (!this.canGrowInto(worldIn.getBlockState(leavesPos.add(k, i, l)).getBlock()) && worldIn.getBlockState(leavesPos.add(k, i, l)).getBlock() != Blocks.SAPLING)) // CraftBukkit - ignore our own saplings
                        {
                            flag = false;
                        }
                    }
