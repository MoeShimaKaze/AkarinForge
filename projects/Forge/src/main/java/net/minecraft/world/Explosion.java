...
public class Explosion
{
    ...
    public final Entity exploder; // Akarin Forge - public
            ...
            if (!entity.isImmuneToExplosions())
            {
                ...
                if (d12 <= 1.0D)
                {
                    ...
                    if (d13 != 0.0D)
                    {
                        ...
                        // CraftBukkit start
                        org.bukkit.craftbukkit.event.CraftEventFactory.entityDamage = exploder;
                        ...
                        if (!wasDamaged && !(entity instanceof EntityTNTPrimed || entity instanceof net.minecraft.entity.item.EntityFallingBlock) && !entity.forceExplosionKnockback) continue;
                        // CraftBukkit end
                        double d11 = d10;
    ...
    public void doExplosionB(boolean spawnParticles)
    {
        ...
        if (this.damagesTerrain)
        {
            ...
            float yield;
            if (explode != null) {
                org.bukkit.event.entity.EntityExplodeEvent event = new org.bukkit.event.entity.EntityExplodeEvent(explode, location, blockList, 1.0F / this.size);
                this.world.getServer().getPluginManager().callEvent(event);
                cancelled = event.isCancelled();
                bukkitBlocks = event.blockList();
                yield = event.getYield();
            } else {
                org.bukkit.event.block.BlockExplodeEvent event = new org.bukkit.event.block.BlockExplodeEvent(location.getBlock(), blockList, 1.0F / this.size);
                this.world.getServer().getPluginManager().callEvent(event);
                cancelled = event.isCancelled();
                bukkitBlocks = event.blockList();
                yield = event.getYield();
            ...
            for (BlockPos blockpos : this.affectedBlockPositions)
            {
                ...
                if (iblockstate.getMaterial() != Material.AIR)
                {
                    ...
                    {
                        block.dropBlockAsItemWithChance(this.world, blockpos, this.world.getBlockState(blockpos), yield, 0); // CraftBukkit - add yield
                    }
        ...
        if (this.causesFire)
        {
            for (BlockPos blockpos1 : this.affectedBlockPositions)
            {
                ...
                {
                    if (!org.bukkit.craftbukkit.event.CraftEventFactory.callBlockIgniteEvent(this.world, blockpos1.getX(), blockpos1.getY(), blockpos1.getZ(), this).isCancelled()) // CraftBukkit - Ignition by explosion
    ...
    public EntityLivingBase getExplosivePlacedBy()
    {
        ...
        {
            return this.exploder instanceof EntityLivingBase ? (EntityLivingBase)this.exploder : (this.exploder instanceof net.minecraft.entity.projectile.EntityFireball ? ((net.minecraft.entity.projectile.EntityFireball) this.exploder).shootingEntity : null); // CraftBukkit - obtain Fireball shooter for explosion tracking
        }
    }
