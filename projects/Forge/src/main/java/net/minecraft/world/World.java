...
public abstract class World implements IBlockAccess, net.minecraftforge.common.capabilities.ICapabilityProvider
{
    ...
    private final org.bukkit.craftbukkit.CraftWorld world;
    ...
    public java.util.ArrayList<org.bukkit.block.BlockState> capturedBlockStates= new java.util.ArrayList<org.bukkit.block.BlockState>(){
        @Override
        public boolean add(org.bukkit.block.BlockState blockState) {
            Iterator<org.bukkit.block.BlockState> blockStateIterator = this.iterator();
            while( blockStateIterator.hasNext() ) {
                org.bukkit.block.BlockState blockState1 = blockStateIterator.next();
    ...
    public List<net.minecraft.entity.item.EntityItem> captureDrops;
    ...
    public org.bukkit.craftbukkit.CraftServer getServer() {
        return (org.bukkit.craftbukkit.CraftServer) org.bukkit.Bukkit.getServer();
    }
    ...
    {
        this.generator = gen;
        this.world = new org.bukkit.craftbukkit.CraftWorld((WorldServer) this, gen, env);
        ...
        getWorldBorder().addListener(new net.minecraft.world.border.IBorderListener() { // From PlayerList#setPlayerManager
            public void onSizeChanged(WorldBorder worldborder, double d0) {
                getServer().getHandle().sendAll(new net.minecraft.network.play.server.SPacketWorldBorder(worldborder, net.minecraft.network.play.server.SPacketWorldBorder.Action.SET_SIZE), worldborder.world);
            }

            public void onTransitionStarted(WorldBorder worldborder, double d0, double d1, long i) {
                getServer().getHandle().sendAll(new net.minecraft.network.play.server.SPacketWorldBorder(worldborder, net.minecraft.network.play.server.SPacketWorldBorder.Action.LERP_SIZE), worldborder.world);
            }

            public void onCenterChanged(WorldBorder worldborder, double d0, double d1) {
                getServer().getHandle().sendAll(new net.minecraft.network.play.server.SPacketWorldBorder(worldborder, net.minecraft.network.play.server.SPacketWorldBorder.Action.SET_CENTER), worldborder.world);
            }

            public void onWarningTimeChanged(WorldBorder worldborder, int i) {
                getServer().getHandle().sendAll(new net.minecraft.network.play.server.SPacketWorldBorder(worldborder, net.minecraft.network.play.server.SPacketWorldBorder.Action.SET_WARNING_TIME), worldborder.world);
            }

            public void onWarningDistanceChanged(WorldBorder worldborder, int i) {
                getServer().getHandle().sendAll(new net.minecraft.network.play.server.SPacketWorldBorder(worldborder, net.minecraft.network.play.server.SPacketWorldBorder.Action.SET_WARNING_BLOCKS), worldborder.world);
            }
    ...
    public boolean setBlockState(BlockPos pos, IBlockState newState, int flags)
    {
        ...
        if (this.captureTreeGeneration) {
            ...
            Iterator<org.bukkit.block.BlockState> it = capturedBlockStates.iterator();
            while (it.hasNext()) {
                org.bukkit.block.BlockState previous = it.next();
            ...
            if (blockstate == null) blockstate = org.bukkit.craftbukkit.block.CraftBlockState.getBlockState(this, pos.getX(), pos.getY(), pos.getZ(), flags);
            blockstate.setTypeId(org.bukkit.craftbukkit.util.CraftMagicNumbers.getId(newState.getBlock()));
        ...
        else
        {
            ...
            if (iblockstate == null)
            {
                ...
                if (this.captureBlockStates) this.capturedBlockStates.remove(blockstate); // CraftBukkit - remove blockstate if failed
                return false;
            }
            else
            {
                ...
                if (blockSnapshot == null && !this.captureBlockStates) // Don't notify clients or update physics while capturing blockstates // CraftBukkit - Modularize client and physic updates
                {
    ...
    public void notifyAndUpdatePhysics(BlockPos pos, @Nullable Chunk chunk, IBlockState iblockstate, IBlockState newState, int flags) { markAndNotifyBlock(pos, chunk, iblockstate, newState, flags); } // Akarin Forge
    ...
    public void neighborChanged(BlockPos pos, final Block blockIn, BlockPos fromPos)
    {
        if (!this.isRemote)
        {
            ...
            {
                // CraftBukkit start
                org.bukkit.craftbukkit.CraftWorld world = ((WorldServer) this).getWorld();
                if (world != null) {
                    org.bukkit.event.block.BlockPhysicsEvent event = new org.bukkit.event.block.BlockPhysicsEvent(world.getBlockAt(pos.getX(), pos.getY(), pos.getZ()), org.bukkit.craftbukkit.util.CraftMagicNumbers.getId(blockIn));
                    this.getServer().getPluginManager().callEvent(event);
                    if (event.isCancelled()) return;
                } // CraftBukkit end // Akarin Forge - TODO Add source block and make mutable
    ...
    public IBlockState getBlockState(BlockPos pos)
    {
        ...
        if (captureTreeGeneration) {
            ...
            while (it.hasNext()) {
                org.bukkit.block.BlockState previous = it.next();
                if (previous.getX() == pos.getX() && previous.getY() == pos.getY() && previous.getZ() == pos.getZ()) {
                    return org.bukkit.craftbukkit.util.CraftMagicNumbers.getBlock(previous.getTypeId()).getStateFromMeta(previous.getRawData());
                }
            }
        } // CraftBukkit end
        if (this.isOutsideBuildHeight(pos))
    ...
    public boolean spawnEntity(Entity entityIn)
    {
        ...
        return addEntity(entityIn, org.bukkit.event.entity.CreatureSpawnEvent.SpawnReason.DEFAULT);
    }
    public boolean addEntity(Entity entityIn, org.bukkit.event.entity.CreatureSpawnEvent.SpawnReason spawnReason) { // Changed signature, added SpawnReason
        if (entityIn == null) return false;
        org.bukkit.event.Cancellable event = null;
        if (entityIn instanceof net.minecraft.entity.EntityLivingBase && !(entityIn instanceof EntityPlayerMP)) {
            boolean isAnimal = entityIn instanceof net.minecraft.entity.passive.EntityAnimal || entityIn instanceof net.minecraft.entity.passive.EntityWaterMob || entityIn instanceof net.minecraft.entity.monster.EntityGolem;
            boolean isMonster = entityIn instanceof net.minecraft.entity.monster.EntityMob || entityIn instanceof net.minecraft.entity.monster.EntityGhast || entityIn instanceof net.minecraft.entity.monster.EntitySlime;
            boolean isNpc = entityIn instanceof net.minecraft.entity.INpc;
            if (spawnReason != org.bukkit.event.entity.CreatureSpawnEvent.SpawnReason.CUSTOM) {
                if (isAnimal && !spawnPeacefulMobs || isMonster && !spawnHostileMobs || isNpc && !getServer().getServer().getCanSpawnNPCs()) {
                    entityIn.isDead = true;
                    return false;
                }
            }
            event = org.bukkit.craftbukkit.event.CraftEventFactory.callCreatureSpawnEvent((net.minecraft.entity.EntityLivingBase) entityIn, spawnReason);
        } else if (entityIn instanceof net.minecraft.entity.item.EntityItem) {
            event = org.bukkit.craftbukkit.event.CraftEventFactory.callItemSpawnEvent((net.minecraft.entity.item.EntityItem) entityIn);
        } else if (entityIn.getBukkitEntity() instanceof org.bukkit.entity.Projectile) {
            event = org.bukkit.craftbukkit.event.CraftEventFactory.callProjectileLaunchEvent(entityIn); // Not all projectiles extend EntityProjectile, so check for Bukkit interface instead
        } else if (entityIn.getBukkitEntity() instanceof org.bukkit.entity.Vehicle){
            event = org.bukkit.craftbukkit.event.CraftEventFactory.callVehicleCreateEvent(entityIn);
        }
    ...
    public void updateEntities()
    {
        ...
        for (this.tickPosition = 0; this.tickPosition < loadedEntityList.size(); ++this.tickPosition) // CraftBukkit - Use field for loop variable
        {
            Entity entity2 = (Entity) this.loadedEntityList.get(this.tickPosition); // CraftBukkit - Use field for loop variable
            ...
            for (int j1 = 0; j1 < this.addedTileEntityList.size(); ++j1)
            {
                ...
                if (!tileentity1.isInvalid())
                {
                    ...
                    if (this.isBlockLoaded(tileentity1.getPos()))
                    {
                        ...
                        // CraftBukkit start - From above, don't screw this up - SPIGOT-1746
                        if (!this.loadedTileEntityList.contains(tileentity1)) {
                            this.addTileEntity(tileentity1);
                        } // CraftBukkit end
                    }
    ...
    public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate)
    {
        if (!(entityIn instanceof EntityPlayer))
        {
            ...
            Chunk startingChunk = this.getChunkIfLoaded(j2, k2 >> 4); // CraftBukkit - check if chunks are loaded as done in previous versions

            boolean isForced = getPersistentChunks().containsKey(new net.minecraft.util.math.ChunkPos(j2 >> 4, k2 >> 4));
            int range = isForced ? 0 : 32;
            boolean canUpdate = !forceUpdate || (startingChunk != null && startingChunk.areNeighborsLoaded(2)); // CraftBukkit
    ...
    public java.util.Map<BlockPos, TileEntity> capturedTileEntities = com.google.common.collect.Maps.newHashMap(); // CraftBukkit
    @Nullable
    ...
    public boolean checkLightFor(EnumSkyBlock lightType, BlockPos pos)
    {
        ...
        if (chunk == null || !chunk.areNeighborsLoaded(1) /*!this.areChunksLoaded(blockposition, 17, false)*/) // Akarin Forge - TODO 16 radius
        // CraftBukkit end
        {
            return false;
        }
    ...
    public int countEntities(Class<?> entityType)
    {
        ...
        for (Entity entity4 : this.loadedEntityList)
        {
            ...
            if (entity4 instanceof EntityLiving) {
                ...
                if (entityinsentient.canDespawn() && entityinsentient.isNoDespawnRequired()) continue;
            } // CraftBukkit end
            if (entityType.isAssignableFrom(entity4.getClass())) // CraftBukkit
            {
                ++j2;
            }
        }
    ...
    public void loadEntities(Collection<Entity> entityCollection)
    {
        ...
        {
            if (entity4 == null) continue; // CraftBukkit
    ...
    public boolean mayPlace(Block blockIn, BlockPos pos, boolean skipCollisionCheck, EnumFacing sidePlacedOn, @Nullable Entity placer)
    {
        ...
        boolean defaultReturn = false; // CraftBukkit - store default return
        if (axisalignedbb != Block.NULL_AABB && !this.checkNoEntityCollision(axisalignedbb.offset(pos), placer))
        {
            defaultReturn = false; // CraftBukkit - store default return
        }
        else if (iblockstate1.getMaterial() == Material.CIRCUITS && blockIn == Blocks.ANVIL)
        {
            defaultReturn = true; // CraftBukkit - store default return
        }
        else
        {
            defaultReturn = iblockstate1.getBlock().isReplaceable(this, pos) && blockIn.canPlaceBlockOnSide(this, pos, sidePlacedOn); // CraftBukkit - store default return
        }
        // CraftBukkit start
        org.bukkit.event.block.BlockCanBuildEvent event = new org.bukkit.event.block.BlockCanBuildEvent(this.getWorld().getBlockAt(pos.getX(), pos.getY(), pos.getZ()), org.bukkit.craftbukkit.util.CraftMagicNumbers.getId(blockIn), defaultReturn);
        this.getServer().getPluginManager().callEvent(event);
    ...
    public EntityPlayer getClosestPlayer(double x, double y, double z, double p_190525_7_, Predicate<Entity> p_190525_9_)
    {
        ...
        for (int j2 = 0; j2 < this.playerEntities.size(); ++j2)
        {
            ...
            if (entityplayer1 == null || entityplayer1.isDead) continue; // CraftBukkit - Fixed an NPE
    ...
    // CraftBukkit start - Calls the method that checks to see if players are sleeping, called by CraftPlayer#setPermanentSleeping
    public void checkSleepStatus() {
        if (!this.isRemote) this.updateAllPlayersSleepingFlag();
    } // CraftBukkit end
