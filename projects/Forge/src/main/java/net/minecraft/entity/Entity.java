...
public abstract class Entity implements ICommandSender, net.minecraftforge.common.capabilities.ICapabilitySerializable<NBTTagCompound>
{
    ...
    public org.bukkit.craftbukkit.entity.CraftEntity getBukkitEntity() {
        if (bukkitEntity == null) bukkitEntity = org.bukkit.craftbukkit.entity.CraftEntity.getEntity(world.getServer(), this);
        return bukkitEntity;
    }
    ...
    public float getBukkitYaw() { return this.rotationYaw; }
    // CraftBukikt end
    ...
    {
        // CraftBukkit start
        if (Float.isNaN(yaw)) yaw = 0; // yaw was sometimes set to NaN, so we need to set it back to 0
        if (yaw == Float.POSITIVE_INFINITY || yaw == Float.NEGATIVE_INFINITY) {
            if (this instanceof EntityPlayerMP) {
                this.world.getServer().getLogger().warning(this.getName() + " was caught trying to crash the server with an invalid yaw");
                ((org.bukkit.craftbukkit.entity.CraftPlayer) this.getBukkitEntity()).kickPlayer("Infinite yaw (Hacking?)"); // Spigot "Nope" -> Descriptive reason
            }
            yaw = 0;
        }
        if (Float.isNaN(pitch)) pitch = 0; // pitch was sometimes set to NaN, so we need to set it back to 0
        if (pitch == Float.POSITIVE_INFINITY || pitch == Float.NEGATIVE_INFINITY) {
            if (this instanceof EntityPlayerMP) {
                this.world.getServer().getLogger().warning(this.getName() + " was caught trying to crash the server with an invalid pitch");
                ((org.bukkit.craftbukkit.entity.CraftPlayer) this.getBukkitEntity()).kickPlayer("Infinite pitch (Hacking?)"); // Spigot "Nope" -> Descriptive reason
            }
            pitch = 0;
        } // CraftBukkit end
    ...
    public void postTick() {
        ...
        if (!this.world.isRemote && this.world instanceof WorldServer) {
            ...
            if (this.inPortal) {
                ...
                if (true || minecraftserver.getAllowNether()) { // CraftBukkit
                    if (!this.isRiding()) {
                        ...
                        if (this.portalCounter++ >= i) {
                            ...
                            byte b0 = (byte) ((this.world.provider.getDimensionType().getId() == -1) ? 0 : -1);
                ...
                this.portalCounter = this.portalCounter > 0 ? this.portalCounter - 4 : 0;
            }
    ...
    protected void setOnFireFromLava()
    {
        if (!this.isImmuneToFire)
        {
            ...
            if (this instanceof EntityLivingBase) {
                if (fire <= 0) {
                    org.bukkit.block.Block damager = null; // not on fire yet, TODO: shouldn't be sending null for the block
                    ...
                    if (!combustEvent.isCancelled()) this.setFire(combustEvent.getDuration());
                } else {
                    this.setFire(15); // This will be called every single tick the entity is in lava, so don't throw an event
                }
                return;
    ...
    public void move(MoverType type, double x, double y, double z)
    {
        ...
        else
        {
            ...
            try {
                this.doBlockCollisions(); // We need to do this regardless of whether or not we are moving thanks to portals
            ...
            if (x == 0 && y == 0 && z == 0 && this.isBeingRidden() && this.isRiding()) return; // Check if we're moving
            // CraftBukkit end
            ...
            if (collidedHorizontally && getBukkitEntity() instanceof org.bukkit.entity.Vehicle) {
                ...
                org.bukkit.block.Block bBlock = this.world.getWorld().getBlockAt(MathHelper.floor(this.posX), MathHelper.floor(this.posY), MathHelper.floor(this.posZ));
                if (d2 > x) {
                    bBlock = bBlock.getRelative(org.bukkit.block.BlockFace.EAST);
                } else if (d2 < x) {
                    bBlock = bBlock.getRelative(org.bukkit.block.BlockFace.WEST);
                } else if (d4 > z) {
                    bBlock = bBlock.getRelative(org.bukkit.block.BlockFace.SOUTH);
                } else if (d4 < z) {
                    bBlock = bBlock.getRelative(org.bukkit.block.BlockFace.NORTH);
                }
                if (bBlock.getType() != org.bukkit.Material.AIR) {
                    org.bukkit.event.vehicle.VehicleBlockCollisionEvent event = new org.bukkit.event.vehicle.VehicleBlockCollisionEvent(vehicle, bBlock);
    ...
    protected void burn(float i) { dealFireDamage(i); } // Akarin Forge
    ...
    public void readFromNBT(NBTTagCompound compound)
    {
        try
        {
            ...
            // CraftBukkit start
            double limit = getBukkitEntity() instanceof org.bukkit.entity.Vehicle ? 100.0D : 10.0D;
            if (Math.abs(this.motionX) > limit) this.motionX = 0.0D;
            if (Math.abs(this.motionY) > limit) this.motionY = 0.0D;
            if (Math.abs(this.motionZ) > limit) this.motionZ = 0.0D;
            // CraftBukkit end
            ...
            if (this instanceof EntityPlayerMP) {
                ...
                String worldName = compound.getString("world"); // TODO: Remove World related checks, replaced with WorldUID
    ...
    public EntityItem entityDropItem(ItemStack stack, float offsetY)
    {
        ...
        else
        {
            ...
            if (this instanceof EntityLivingBase && !((EntityLivingBase) this).forceDrops) {
                ((EntityLivingBase) this).drops.add(org.bukkit.craftbukkit.inventory.CraftItemStack.asBukkitCopy(stack));
                return null;
            } // CraftBukkit end
    ...
    protected void addPassenger(Entity passenger)
    {
        ...
        else
        {
            ...
            if (getBukkitEntity() instanceof org.bukkit.entity.Vehicle && passenger.getBukkitEntity() instanceof org.bukkit.entity.LivingEntity && passenger.world.isChunkLoaded((int) passenger.posX >> 4, (int) passenger.posZ >> 4, false)) { // Boolean not used
                ...
                if (event.isCancelled() || n != orig) return;
            } // CraftBukkit end
    ...
    public void onStruckByLightning(EntityLightningBolt lightningBolt)
    {
        ...
        if (thisBukkitEntity instanceof org.bukkit.entity.Hanging) {
            ...
            if (hangingEvent.isCancelled()) return;
        }
        ...
        if (this.fire == 0)
        {
            ...
            if (!entityCombustEvent.isCancelled()) this.setFire(entityCombustEvent.getDuration());
            // CraftBukkit end
        }
    }
        ...
        if (!this.world.isRemote && !this.isDead)
        {
            ...
            if (this.dimension < org.bukkit.craftbukkit.CraftWorld.CUSTOM_DIMENSION_OFFSET) { // Plugins must specify exit from custom Bukkit worlds
                ...
                for (WorldServer world : minecraftserver.worlds) {
                    if (world.dimension == dimensionIn) exitWorld = world;
                }
            }
            ...
            org.bukkit.TravelAgent agent = exit != null ? (org.bukkit.TravelAgent) teleporter : org.bukkit.craftbukkit.CraftTravelAgent.DEFAULT; // return arbitrary TA to compensate for implementation dependent plugins // Akarin Forge - respect Forge
            ...
            Entity entity = this.teleportTo(exit, true, teleporter);
            this.world.profiler.endSection();
            return entity;
        }
        return null;
    }
    public Entity teleportTo(org.bukkit.Location exit, boolean portal, net.minecraftforge.common.util.ITeleporter teleporter) {
        if (true) {
            ...
            int dimensionIn = worldserver1.dimension;
            // CraftBukkit end
            ...
            */ // CraftBukkit end
            ...
            worldserver1.getMinecraftServer().getPlayerList().repositionEntity(this, exit, portal); // Akarin Forge - FIXME respect Forge teleporter
            // CraftBukkit end
            ...
            if (entity != null)
            {
                ...
                if (this instanceof EntityLiving) ((EntityLiving)this).clearLeashed(true, false); // Unleash to prevent duping of leads.
                // CraftBukkit end
            }
    ...
    {
        if (name.length() > 256) name = name.substring(0, 256); // CraftBukkit - Add a sane limit for name length
    ...
    public void setEntityBoundingBox(AxisAlignedBB bb)
    {
        ...
        double mixX = bb.minX, minY = bb.minY, minZ = bb.minZ,
                maxX = bb.maxX, maxY = bb.maxY, maxZ = bb.maxZ;
        double len = bb.maxX - bb.minX;
        if (len < 0) maxX = mixX;
        if (len > 64) maxX = mixX + 64.0;
        ...
        this.boundingBox = new AxisAlignedBB(mixX, minY, minZ, maxX, maxY, maxZ);
        // CraftBukkit end
    }
