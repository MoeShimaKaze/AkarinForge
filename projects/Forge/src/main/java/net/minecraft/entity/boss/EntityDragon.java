...
public class EntityDragon extends EntityLiving implements IEntityMultiPart, IMob
{
    ...
    private net.minecraft.world.Explosion explosionSource = new net.minecraft.world.Explosion(null, this, Double.NaN, Double.NaN, Double.NaN, Float.NaN, true, true); // CraftBukkit - reusable source for CraftTNTPrimed.getSource()
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (vec3d != null && iphase.getType() != PhaseList.HOVER) // CraftBukkit - Don't move when hovering // PAIL: rename
                    {
    ...
    private boolean destroyBlocksInAABB(AxisAlignedBB p_70972_1_)
    {
        ...
        if (event.isCancelled()) {
            ...
            return flag;
        } else if (event.getYield() == 0F) { // Yield zero ==> no drops
            ...
            for (org.bukkit.block.Block block : event.blockList()) {
                ...
                if (blockId == org.bukkit.Material.AIR) continue;
