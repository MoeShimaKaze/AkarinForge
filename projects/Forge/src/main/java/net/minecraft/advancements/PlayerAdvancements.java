        ...
        if (this.progressFile.isFile())
        {
            try
            {
                ...
                for (Entry<ResourceLocation, AdvancementProgress> entry : stream.collect(Collectors.toList()))
                {
                    ...
                    if (advancement == null)
                    {
                        if (((ResourceLocation) entry.getKey()).getResourceDomain().equals("minecraft")) LOGGER.warn("Ignored advancement '" + entry.getKey() + "' in progress file " + this.progressFile + " - it doesn't exist anymore?");
                    }
                    else
                    {
