...
public class FunctionManager implements ITickable
{
    ...
    public class CustomFunctionListener implements ICommandSender {
        ...
        {
            return FunctionManager.this.server.bworlds.get(0); // CraftBukkit
        }
    ...
    {
        return this.server.bworlds.get(0).getGameRules().getInt("maxCommandChainLength"); // CraftBukkit
    }
    ...
    public void update()
    {
        String s = this.server.bworlds.get(0).getGameRules().getString("gameLoopFunction"); // CraftBukkit
