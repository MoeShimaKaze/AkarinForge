...
public abstract class BlockButton extends BlockDirectional
{
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        else
        {
            ...
            if ((eventRedstone.getNewCurrent() > 0) != (!powered)) return true;
            // CraftBukkit end
    ...
    private void checkPressed(IBlockState state, World worldIn, BlockPos pos)
    {
        ...
        if (flag1 != flag && flag) {
            ...
            for (Object object : list) { // If all of the events are cancelled block the button press, else allow
        ...
        if (flag && !flag1)
        {
            // CraftBukkit start
            org.bukkit.block.Block block = worldIn.getWorld().getBlockAt(pos.getX(), pos.getY(), pos.getZ());
            org.bukkit.event.block.BlockRedstoneEvent eventRedstone = new org.bukkit.event.block.BlockRedstoneEvent(block, 0, 15);
            worldIn.getServer().getPluginManager().callEvent(eventRedstone);
            if (eventRedstone.getNewCurrent() <= 0) return;
            // CraftBukkit end
