...
public class BlockFarmland extends Block
{
    ...
    public void onFallenUpon(World worldIn, BlockPos pos, Entity entityIn, float fallDistance)
    {
        ...
        if (!worldIn.isRemote && entityIn.canTrample(worldIn, this, pos, fallDistance)) // Forge: Move logic to Entity#canTrample
        {
            ...
            if (entityIn instanceof EntityPlayer) {
                cancellable = org.bukkit.craftbukkit.event.CraftEventFactory.callPlayerInteractEvent((EntityPlayer) entityIn, org.bukkit.event.block.Action.PHYSICAL, pos, null, null, null);
            } else {
                cancellable = new org.bukkit.event.entity.EntityInteractEvent(entityIn.getBukkitEntity(), worldIn.getWorld().getBlockAt(pos.getX(), pos.getY(), pos.getZ()));
                worldIn.getServer().getPluginManager().callEvent((org.bukkit.event.entity.EntityInteractEvent) cancellable);
            }
            if (cancellable.isCancelled()) return;
            if (org.bukkit.craftbukkit.event.CraftEventFactory.callEntityChangeBlockEvent(entityIn, pos, Blocks.DIRT, 0).isCancelled()) return;
            // CraftBukkit end // Akarin Forge - TODO Move logic to Entity#canTrample
            turnToDirt(worldIn, pos);
        }
    ...
    protected static void turnToDirt(World p_190970_0_, BlockPos worldIn)
    {
        ...
        if (org.bukkit.craftbukkit.event.CraftEventFactory.callBlockFadeEvent(block, Blocks.DIRT).isCancelled()) return;
        // CraftBukkit end
