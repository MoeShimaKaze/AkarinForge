...
public class BlockPortal extends BlockBreakable
{
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        ...
        if (worldIn.provider.isSurfaceWorld() && worldIn.getGameRules().getBoolean("doMobSpawning") && rand.nextInt(2000) < worldIn.getDifficulty().getDifficultyId())
        {
            ...
            {
                Entity entity = ItemMonsterPlacer.spawnCreature(worldIn, EntityList.getKey(EntityPigZombie.class), (double)blockpos.getX() + 0.5D, (double)blockpos.getY() + 1.1D, (double)blockpos.getZ() + 0.5D, org.bukkit.event.entity.CreatureSpawnEvent.SpawnReason.NETHER_PORTAL); // CraftBukkit - set spawn reason to NETHER_PORTAL
    ...
    public boolean trySpawnPortal(World worldIn, BlockPos pos)
    {
        ...
        {
            return blockportal$size.placePortalBlocks(); // CraftBukkit
        }
        else
        {
            ...
            {
                return blockportal$size1.placePortalBlocks(); // CraftBukkit
            }
            else
            {
                return false;
    ...
    public static class Size
        {
                ...
                for (this.height = 0; this.height < 21; ++this.height)
                {
                    for (int i = 0; i < this.width; ++i)
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            {
                                break label56;
                            } else { // CraftBukkit start - add the block to our list
            ...
            public boolean placePortalBlocks() // CraftBukkit start - return boolean
            {
                org.bukkit.World bworld = this.world.getWorld();
                for (int i = 0; i < this.width; ++i) { // Copy below for loop
