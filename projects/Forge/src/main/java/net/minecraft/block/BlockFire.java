...
public class BlockFire extends Block
{
                                ...
                                if (k1 > 0)
                                {
                                    ...
                                    if (l1 > 0 && rand.nextInt(j1) <= l1 && (!worldIn.isRaining() || !this.canDie(worldIn, blockpos)))
                                    {
                                        ...
                                        if (worldIn.getBlockState(blockpos) != Blocks.FIRE) {
                                            if (org.bukkit.craftbukkit.event.CraftEventFactory.callBlockIgniteEvent(worldIn, blockpos.getX(), blockpos.getY(), blockpos.getZ(), pos.getX(), pos.getY(), pos.getZ()).isCancelled()) {
                                                continue;
                                            }
                                            ...
                                            org.bukkit.event.block.BlockSpreadEvent spreadEvent = new org.bukkit.event.block.BlockSpreadEvent(blockState.getBlock(), bworld.getBlockAt(pos.getX(), pos.getY(), pos.getZ()), blockState);
                                            server.getPluginManager().callEvent(spreadEvent);
                                            if (!spreadEvent.isCancelled()) blockState.update(true);
                                        } // CraftBukkit end
                                    }
                                }
                            }
    ...
    private void tryCatchFire(World worldIn, BlockPos pos, int chance, Random random, int age, EnumFacing face) { this.tryCatchFire(worldIn, pos, chance, random, age, face, null); } // Akarin Forge
    private void tryCatchFire(World worldIn, BlockPos pos, int chance, Random random, int age, EnumFacing face, BlockPos source) // Akarin Forge
    {
        ...
        if (random.nextInt(chance) < i)
        {
            ...
            // CraftBukkit start
            if (source == null) source = pos.offset(face.getOpposite()); // Akarin Forge
            org.bukkit.block.Block theBlock = worldIn.getWorld().getBlockAt(pos.getX(), pos.getY(), pos.getZ());
            org.bukkit.block.Block sourceBlock = worldIn.getWorld().getBlockAt(source.getX(), source.getY(), source.getZ());
    ...
    private void fireExtinguished(World world, BlockPos position) {
        if (!org.bukkit.craftbukkit.event.CraftEventFactory.callBlockFadeEvent(world.getWorld().getBlockAt(position.getX(), position.getY(), position.getZ()), Blocks.AIR).isCancelled()) {
