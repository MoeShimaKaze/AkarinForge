            ...
            if (worldIn.isAirBlock(blockpos) && blockpos.getY() < 256)
            {
                ...
                if (i < 5 &&  net.minecraftforge.common.ForgeHooks.onCropsGrowPre(worldIn, blockpos, state, rand.nextInt(1) == 0))
                {
                    ...
                    if (flag && areAllNeighborsEmpty(worldIn, blockpos, (EnumFacing)null) && worldIn.isAirBlock(pos.up(2)))
                    {
                        ...
                        if (org.bukkit.craftbukkit.event.CraftEventFactory.handleBlockSpreadEvent(
                                ...
                                this, getMetaFromState(this.getDefaultState().withProperty(AGE, Integer.valueOf(i)))
                        )) {
                    ...
                    else if (i < 4)
                    {
                        ...
                        for (int j1 = 0; j1 < l; ++j1)
                        {
                            ...
                            if (worldIn.isAirBlock(blockpos1) && worldIn.isAirBlock(blockpos1.down()) && areAllNeighborsEmpty(worldIn, blockpos1, enumfacing.getOpposite()))
                            {
                                ...
                                if (org.bukkit.craftbukkit.event.CraftEventFactory.handleBlockSpreadEvent(
                                        ...
                                        worldIn.getWorld().getBlockAt(pos.getX(), pos.getY(), pos.getZ()),
                                        this, getMetaFromState(this.getDefaultState().withProperty(AGE, Integer.valueOf(i + 1)))
                                )) {
                                    worldIn.playEvent(1033, pos, 0);
                                    flag2 = true;
                                } // CraftBukkit end
                        ...
                        else
                        {
                            ...
                            if (org.bukkit.craftbukkit.event.CraftEventFactory.handleBlockGrowEvent(
                                    worldIn, pos.getX(), pos.getY(), pos.getZ(), this, getMetaFromState(state.withProperty(BlockChorusFlower.AGE, Integer.valueOf(5)))
                            )) {
