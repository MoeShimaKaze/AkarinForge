...
public class BlockSkull extends BlockContainer
{
    ...
    @Override public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune) {
        List<ItemStack> drops = getDrops(worldIn, pos, state, fortune); // Akarin Forge - respect Forge
        if (worldIn.rand.nextFloat() < net.minecraftforge.event.ForgeEventFactory.fireBlockHarvesting(drops, worldIn, pos, state, fortune, chance, false, harvesters.get())) {
            TileEntity tileentity = worldIn.getTileEntity(pos);
            if (tileentity instanceof TileEntitySkull) {
    ...
    {
        if (worldIn.captureBlockStates) return; // CraftBukkit
        if (te.getSkullType() == 1 && pos.getY() >= 2 && worldIn.getDifficulty() != EnumDifficulty.PEACEFUL && !worldIn.isRemote)
        {
            ...
            {
                org.bukkit.craftbukkit.util.BlockStateListPopulator blockList = new org.bukkit.craftbukkit.util.BlockStateListPopulator(worldIn.getWorld()); // CraftBukkit
                for (int i = 0; i < 3; ++i)
                {
                    BlockWorldState blockworldstate = blockpattern$patternhelper.translateOffset(i, 0, 0);
                    // CraftBukkit start
                    BlockPos blockpos = blockworldstate.getPos();
                    IBlockState data = blockworldstate.getBlockState().withProperty(NODROP, Boolean.valueOf(true));
                ...
                if (worldIn.addEntity(entitywither, org.bukkit.event.entity.CreatureSpawnEvent.SpawnReason.BUILD_WITHER)) { // CraftBukkit
                blockList.updateList(); // CraftBukkit
